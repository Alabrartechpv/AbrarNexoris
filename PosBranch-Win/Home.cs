...
namespace PosBranch_Win
{
    public partial class Home : Form
    {
        ...
        public Home()
        {
            ...
            // Customize Ribbon: Rename TradingPLAccount to Trading Account and add ProfitLossAccount button dynamically
            try
            {
                if (ultraToolbarsManager1.Tools.Exists("TradingPLAccount"))
                {
                    var tool = ultraToolbarsManager1.Tools["TradingPLAccount"] as Infragistics.Win.UltraWinToolbars.ButtonTool;
                    if (tool != null)
                    {
                        tool.SharedProps.Caption = "Trading Account";
                        tool.SharedProps.ToolTipText = "Trading Account";
                        tool.SharedProps.ToolTipTitle = "Trading Account";
                    }
                }

                if (!ultraToolbarsManager1.Tools.Exists("ProfitLossAccount"))
                {
                    var profitLossTool = new Infragistics.Win.UltraWinToolbars.ButtonTool("ProfitLossAccount");
                    profitLossTool.SharedProps.Caption = "Profit & Loss Account";
                    profitLossTool.SharedProps.ToolTipText = "Profit & Loss Account";
                    profitLossTool.SharedProps.ToolTipTitle = "Profit & Loss Account";
                    profitLossTool.SharedProps.DisplayStyle = Infragistics.Win.UltraWinToolbars.ToolDisplayStyle.ImageAndText;
                    
                    if (global::PosBranch_Win.Properties.Resources.profit_and_loss != null)
                    {
                        profitLossTool.SharedProps.AppearancesLarge.Appearance.Image = global::PosBranch_Win.Properties.Resources.profit_and_loss;
                    }

                    ultraToolbarsManager1.Tools.Add(profitLossTool);

                    var ribbonTab = ultraToolbarsManager1.Ribbon.Tabs.Exists("ribbon3") ? ultraToolbarsManager1.Ribbon.Tabs["ribbon3"] : null;
                    if (ribbonTab != null)
                    {
                        var ribbonGroup = ribbonTab.Groups.Exists("FinancialReports") ? ribbonTab.Groups["FinancialReports"] : null;
                        if (ribbonGroup != null)
                        {
                            int tradingIndex = -1;
                            for (int i = 0; i < ribbonGroup.Tools.Count; i++)
                            {
                                if (ribbonGroup.Tools[i].Key == "TradingPLAccount")
                                {
                                    tradingIndex = i;
                                    break;
                                }
                            ...
                            if (tradingIndex >= 0)
                            {
                                var instanceTool = ribbonGroup.Tools.InsertTool(tradingIndex + 1, "ProfitLossAccount");
                            ...
                            {
                                var instanceTool = ribbonGroup.Tools.AddTool("ProfitLossAccount");
                                instanceTool.InstanceProps.PreferredSizeOnRibbon = Infragistics.Win.UltraWinToolbars.RibbonToolSize.Large;
                                instanceTool.InstanceProps.MinimumSizeOnRibbon = Infragistics.Win.UltraWinToolbars.RibbonToolSize.Large;
                            }
                        }
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Error adding ProfitLossAccount tool dynamically: {ex.Message}");
            }
        }
