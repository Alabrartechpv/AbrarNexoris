...
BEGIN TRY
    ...
    BEGIN
        ...
        SELECT 
            -- Sales Summary (using SMaster columns)
            ISNULL(SUM(SM.SubTotal), 0) AS TotalGrossSales,
            ISNULL(SUM(SM.DiscountAmt), 0) AS TotalDiscount,
            0.00 AS TotalReturn, -- Returns handled separately
            ISNULL(SUM(SM.NetAmount), 0) AS NetSales,
            
            -- Payment Collection by Type (using PaymodeName)
            ...
            -- Total Collection (using ReceivedAmount)
            ISNULL(SUM(SM.ReceivedAmount), 0) AS TotalCollection,
            
            -- Transaction Counts (using BillNo)
    ...
    -- TEMPORARY: Returns zeros until we identify correct table structure
    -- ==========================================
    ELSE IF(@_Operation = 'GETCUSTOMERRECEIPTS')
    BEGIN
        -- Return zeros for now (customer receipts handled separately)
        -- TODO: Update this when customer receipt table structure is identified
        SELECT 
            0.00 AS CashReceipt,
            0.00 AS CardReceipt,
            0.00 AS UpiReceipt,
...
PRINT '✅ Now using SMaster table with correct column names'
PRINT ''
PRINT 'Column Mapping:'
PRINT '  GrossAmount → SubTotal'
PRINT '  DiscountAmount → DiscountAmt'
PRINT '  PaidAmount → ReceivedAmount'
PRINT '  PaymentMode → PaymodeName'
PRINT '  SaleDate → BillDate'
PRINT '  SaleId → BillNo'
PRINT ''
PRINT 'Test with:'
PRINT 'EXEC POS_ShiftClosing'
PRINT '  @_Operation = ''GETSALESDATASUMMARY'','
PRINT '  @BranchId = 1,'
PRINT '  @CompanyId = 1,'
PRINT '  @FinYearId = 1,'
PRINT '  @ClosingDate = ''2025-12-05'','
PRINT '  @UserId = 1,'
PRINT '  @Counter = ''admin'''
GO
